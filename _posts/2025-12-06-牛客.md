...
## 使用set的时候，注意不要边插入边遍历这个set。如果需要修改可以使用一个临时的set
...
最好是使用vector
```cpp title:test.cpp hl:2-5 hlt:row
因为vector

std::vector<std::vector<int>> a(5, std::vector<int>(4, 5));

std::vector<std::vector<int>> matrix(3, std::vector<int>(4, 5)); // zhe 

这样就初始化了一个row = 5， col = 4， 值都为 5的二维数组。

vector的构造函数，第一个参数是大小，第二个参数是每个元素的值。
```



## dfs遍历的时候，最好先探测，不要等真的走到了再去回头